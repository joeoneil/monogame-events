...
{ 
  ...
  private struct ButtonEvent
  {
    ...
    public ButtonEvent(Buttons button, Action callback, PressType pressType)
    {
      this.button = button;
  ...
  public void Update(GamePadState newState)
  {
    ...
    foreach (var handle in handles)
    {
      ...
      {
        case PressType.Held:
          if (IsHeld(handle.button)) handle.callback();
          break;
        case PressType.Pressed:
          if (IsPressed(handle.button)) handle.callback();
          break;
        case PressType.Released:
          if (IsReleased(handle.button)) handle.callback();
          break;
      }
    }
  }
  ...
  public void OnPressed(Buttons button, Action callback)
  {
    AddCallback(button, callback, PressType.Pressed);
  }

  public void OnHeld(Buttons button, Action callback)
  {
    AddCallback(button, callback, PressType.Held);
  }

  public void OnReleased(Buttons button, Action callback)
  {
    AddCallback(button, callback, PressType.Released);
  }

  private void AddCallback(Buttons button, Action callback, PressType pressType)
  {
    handles.Insert(0, new ButtonEvent(button, callback, pressType));
  }
}